...
export default {
  ...
  methods: {
    a() {
      ...
      let maxnum = Math.max(...this.chartData.map((item)=>Number(item.index_val)))>100?Math.max(...this.chartData.map((item)=>Number(item.index_val)))>1000?800:100:20;
      let minnum = Math.min(...this.chartData.map((item)=>Number(item.index_val)))>100?Math.min(...this.chartData.map((item)=>Number(item.index_val)))>1000?-600:-50:-10;
            ...
            areaStyle:{
              normal: {
                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                    offset: 0,
                    color: this.ListItem.LastYear>0?'rgba(246,234,237,0.4)':'rgba(34,160,129,0.4)'
                }, {
                    offset: 0.9,
                    color: this.ListItem.LastYear>0?'rgba(233,73,86,0)':'rgba(232,242,241,0)'
