<template>
  ...
  <!-- ---------------移动端首页------------------ -->
    <!-- 头部结构 -->
    <!-- --------------结构2---------------- -->
    <div class="tab-back h40rem rowstart relative">
      <img class="absolute" :style="tabSelectStyles" :src="require(`@/images/${tabImages[selectTabIndex]}.png`)" alt="" />
      <div class="move-line absolute" :style="tabMoveLineStyles"></div>
      <div v-for="(item, index) in tabs" :key="index" class="columncenter point tab-item" style="z-index: 9" @click="tabChange(item, index)">
        <div class="font16rem fw500" :class="index == selectTabIndex ? 'c191919': 'c9a571f'">{{ item }}</div>
      </div>
    </div>
  ...
  <!-- 导航栏 -->
    ...
    <!-- 导航栏下展示内容 -->
    <div v-for="(item, index) in icons" :key="index" class="tagsItem" @click="actionPage(item)">
      <img
        style="width: 40px; height: 40px"
